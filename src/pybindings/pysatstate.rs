...
impl PySatState {
    ...
    fn set_pvh_pos_uncertainty(&mut self, sigma_pvh: &np::PyArray1<f64>) -> PyResult<()> {
        ...
        let na_sigma_pvh = na::Vector3::<f64>::from_row_slice(unsafe { sigma_pvh.as_slice().unwrap() });
    ...
    fn set_cartesian_pos_uncertainty(&mut self, sigma_cart: &np::PyArray1<f64>) -> PyResult<()> {
        ...
        let na_sigma_cart = na::Vector3::<f64>::from_row_slice(unsafe { sigma_cart.as_slice().unwrap() });
        ...
        pyo3::Python::with_gil(|py| -> Py<PyAny> {
            match self.inner.cov {                
                ...
                StateCov::PVCov(cov) => {
                    ...
                    np::PyArray1::from_slice(py, cov.as_slice()).reshape(dims).unwrap().to_object(py)
                }
