...
impl RKAdaptive<21, 8> for RKV98 {
    ...
    const B: [f64; 21] = rkv98_table::B;

    const C: [f64; 21] = rkv98_table::C;

    const A: [[f64; 21]; 21] = rkv98_table::A;

    const BI: [[f64; 8]; 21] = rkv98_table::BI;

    const BERR: [f64; 21] = {
        let mut berr = [0.0; 21];
        let mut ix: usize = 0;
        while ix < 21 {
            berr[ix] = Self::B[ix] - rkv98_table::BHAT[ix];
            ix += 1;
        }
        berr
    };
